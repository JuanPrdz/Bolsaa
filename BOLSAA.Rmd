...
So <- as.numeric(na.omit(precio[n,1]))
dt <- data_frame(10,20,40)
epsilon <- rnorm(10000)
dss <- 0
for(i in length(dt)){
   dss <- data.frame(
  mu_d*So*as.numeric(dt[i]) + sigma_d*So*epsilon*sqrt(as.numeric(dt[i])))
}
dss
...
dss <- data.frame(mu_d*So*as.numeric(dt[1]) + sigma_d*So*epsilon*sqrt(as.numeric(dt[1])))
...
```{r}
Esperado_Cambio <- mean(ds)
Varianza_Cambio <- var(ds)
Des_Cambio <- sqrt(Varianza_Cambio)
Esperado_Cambio
Varianza_Cambio
```

```{r}
P_Esperado <- So+Esperado_Cambio
P_Esperado
```

```{r}
Z <- qnorm(0.025)
P_Esperado-Z*Des_Cambio
P_Esperado+Z*Des_Cambio
```
...
```{r}
Intervalo3m <- tibble(LimiteInf=P_Esperado3m+Z3m*Des_Cambio3m, LimiteSUp=P_Esperado3m-Z3m*Des_Cambio3m)

kable(Intervalo3m, caption="<center>3 meses 95% de confianza", align=c("c","c","c"))%>%
  kable_classic(full_width=FALSE, html_font="Cambria")%>%
  row_spec(0,bold=T, color="red")%>%
  column_spec(1)
```
...
```{r}
Intervalo6m <- tibble(LimiteInf=P_Esperado6m+Z6m*Des_Cambio6m, LimiteSUp=P_Esperado6m-Z6m*Des_Cambio6m)

kable(Intervalo6m, caption="<center>6 meses 95% de confianza", align=c("c","c","c"))%>%
  kable_classic(full_width=FALSE, html_font="Cambria")%>%
  row_spec(0,bold=T, color="red")%>%
  column_spec(1)
```
...
```{r}
Intervalo9m <- tibble(LimiteInf=P_Esperado9m+Z9m*Des_Cambio9m, LimiteSUp=P_Esperado9m-Z9m*Des_Cambio9m)

kable(Intervalo9m, caption="<center>9 meses 95% de confianza", align=c("c","c","c"))%>%
  kable_classic(full_width=FALSE, html_font="Cambria")%>%
  row_spec(0,bold=T, color="red")%>%
  column_spec(1)
```

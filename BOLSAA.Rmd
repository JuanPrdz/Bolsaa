...
[Simec](https://www.emis.com/) es una mini-acería fabricante de productos de acero no planos con una amplia gama de perfiles estructurales, comerciales, varilla y aceros especiales, que se emplean principalmente en los sectores de la construcción, automotriz y manufactura.
...
Esto nos dice que nuestro portafolio para el día `r dia_inicial` es de `r vi`

```{r,include=FALSE}
datos6m <- new.env()
clave6m <- "BOLSAA.MX"
getSymbols(clave6m, from="2021-05-19", to=today(), warnings=FALSE, env=datos6m)
activo6m <- na.omit(datos6m[[clave6m]])
precio6m <- activo6m[,6]
...
activo26m <- na.omit(datos26m[[clave26m]])
...
plot(rend6m)
```
En las gráficas superiores podemos ver los precios y los rendimientos de ambos activos con los últimos 6 meses. 
...
Después procedimos a crear una gráfica de la estimación del valor del portafolio para el siguiente día.
```{r, echo=FALSE}
...
Podemos observar que la mayoría de las simulaciones se ubican entre 18500 y 21500 en la gráfica
Con una media de aproximadamente 18000
...
En la grafica observamos la estimación de pérdidas y ganacias donde tenemos una media realmente muy cercana a 0, ni pérdida ni ganancia y la mayor parte de las simulaciones se ubican entre -1500 y 1500. Además agregamos una segunda gráfica con su distribución.
...
Considerando una confianza de 90%, la estimación del VaR fue de aproximadamente 536.64 de pérdida para 1 día y de 2399.90 de pérdida para 20 días 
...
**Paso 1: Construir el valor real del portafolio en el tiempo**
...
En la gráfica previa se observan en picos donde el VaR superó la estimación y gráficamente sólo se ve 1.
```{r, include=FALSE}
...
Finalmente en esta tabla ordenamos el backtesting para un día:
...
El VaR no paramétrico no asume normalidad, utiliza los históricos y los proyecta a futuro, así, estimando el VaR para los mísmos días futuros que los pasados. 
...
```
Podemos observar que la mayoría de las simulaciones se ubican entre 18000 y 22000 en la gráfica
Con una media de aproximadamente 19800.
...
Y ahora en la siguiente tabla, observaremos como son los VaR para 1, 5, 10 y 20 días. 
```{r, include=FALSE}
# 1 dia
...
Acomodamos en tabla los VaR con sus respectivos niveles de confianza y días futuros estimados. 
```{r}
VaR_Totalo
...
plot(backtesting_99)
```
Aquí se ven un poco más de irregularidades con las proyecciones, pero realmente son pocas.
...
```{r, echo=FALSE}
cat("Porcentaje de error 99%: ", back99)
cat(", Porcentaje de error 95%: ", back95)
cat(" y Porcentaje de error 90%: ", back90)
```

Se raliza el backtesting y podemos observar que nuestra medición es bastante buena, fallando para el 99% solo el 0.2% de las veces y para el 90% solo el 2% de las veces por tanto podemos afrmar que nuetro test es satisfactorio, y nuestra medida de var es correcta 
...
Ponderaciónes de los activos 

```{r}
# Correlación de los activos: -0.01458363
w1 <- (2*vol_2^2-2*-0.01458363)/(2*vol_1^2-4*-0.01458363	+ 2*vol_2^2)
w2 <- 1-w1
w1
w2
```
Para los activos descritos Bolsa y SIMECB si tuvieramos un portafolio 
la ponderación adecuada para reducir la minima varianza es de 49.9% para bolsa 
y 50.1% para SIMECB 



# Conclusión: 
Con la ayuda del VaR podemos tener conocimiento de cuánto es el máximo valor posible de párdida de nuestro portafolio para diferentes periodos de tiempo. Nos da una información vital a la hora de adquirir cierta acción o portafolio. 
Nos parecen ambas metodologías útiles, aunque da menor nivel de error en el backtesting el VaR no paramétrico da mejores medidas y estimaciones del VaR.
Con todo este proyecto, pudimos analizar los diferente métodos y medidas que nos ayudan a estudiar el riesgo que conlleva adquirir algun activo financiero. Ya sea con suposiciones de normalidad o no. Fue muy interesante estudiar este activo debido a que aprendimos a verlo de otra manera más allá de sálo comprar o vender dependiendo de su gráfica. Ahora tenemos herramientas para elegir de manera óptima los activos de nuestro portafolio, teniendo en cuenta el riesgo que conllevan. 

También aprendimos a determinar volatilidades, para uno o mas días con diferentes métodologias. Lo cuál tambien podemos aplicar de ahora en adelante, día con día en nuestros portafolios personales. 

Juan Pablo Rodriguez
Iván Arellano Ruelas
Santiago Romero

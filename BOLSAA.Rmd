...
Para elegir el activo de menor correlación utlizarameos el criterio de corelación y eligiremos el que menos corelación tiene para poder diversificar mas nuestro riesgo
```{r}
menos_correlacion <- arrange(Resultados, V2)[2:6,]
menos_correlacion
```

Observamos que el que menos correlación tiene con nuestro activo es 'FRES.MX'
Ques una empresa de minera en la Republica Mexicana con sede en la CDMX
...
datos2 <- new.env()
clave2 <- "FRES.MX"
getSymbols(clave2, from="2016-01-01", to=today(), warnings=FALSE, env=datos)
activo2 <- datos[[clave2]]
precio2 <- activo2[,6]
...
Sacamos los rendimientos de los activos y los observamos en una gráfica.
```{r}
...
Construyamos un portafolio y determinemos su valor inicial. (Su valor el día de hoy)
...
pe_a1 <- pi_a1*exp((mu_1-.5*vol_1^2)*dt+vol_1*epsilon*sqrt(dt))
pe_a2 <- pi_a2*exp((mu_2-.5*vol_2^2)*dt+vol_2*epsilon*sqrt(dt))
```

```{r}
dia_est <- dia_inicial+1
vp_est <- pe_a1*a1+pe_a2*a2
plot(vp_est, main=paste("Valor Estimado del Portafolio para el día", dia_est), ylab="Pesos", xlab="Escenarios")
abline(h=vi, col="red", lwd=2)
```

```{r}
fpg <- vp_est-vi
plot(fpg, main = paste("Pérdida/Ganancia Estimada", dia_est), ylab="Pesos", xlab="Escenarios")
abline(h=0, col="red", lwd=2)
hist(fpg, n=50, main=paste("Pérdida/Ganancia Estimada", dia_est))
```

```{r}
# 1 dia
VaR99 <- as.numeric(quantile(fpg,0.01))
VaR95 <- as.numeric(quantile(fpg,0.05))
VaR90 <- as.numeric(quantile(fpg,0.1))

# 5 dias
VaR99_5 <- as.numeric(quantile(fpg,0.01)*sqrt(5))
VaR95_5 <- as.numeric(quantile(fpg,0.05)*sqrt(5))
VaR90_5 <- as.numeric(quantile(fpg,0.1)*sqrt(5))

# 10 dias
VaR99_10 <- as.numeric(quantile(fpg,0.01)*sqrt(10))
VaR95_10 <- as.numeric(quantile(fpg,0.05)*sqrt(10))
VaR90_10 <- as.numeric(quantile(fpg,0.1)*sqrt(10))

# 20 dias
VaR99_20 <- as.numeric(quantile(fpg,0.01)*sqrt(20))
VaR95_20 <- as.numeric(quantile(fpg,0.05)*sqrt(20))
VaR90_20 <- as.numeric(quantile(fpg,0.1)*sqrt(20))
...
VaR_Total <- data.frame("VaR al 99%"=VAR99, "VaR al 95%"=VAR95, "VaR al 90%"=VAR90, row.names = DIAS)
VaR_Total

PRUEBAf <- data.frame("VaR al 99%"=VaR99)
PRUEBAf
```

```{r}
VaR90 <- as.numeric(VaR90)
VaR90_10 <- as.numeric(VaR90_10)
```


```{r}
BAR <- c(VaR90,VaR90_10)
dias <- c(dia_est,dia_est+1)
```
